...
青简自绘外观仍在兼容性验证中。普通安装包含 X11/XWayland 自绘窗口，设置 `qingjian` 后会尝试使用；窗口无法定位、桌面不支持或绘制失败时自动恢复 Fcitx5 面板。`auto` 暂不启用自绘，原生 Wayland 应用和 GNOME 搜索也继续使用 Fcitx5 面板。出现显示问题时改回 `fcitx` 即可恢复默认外观。
