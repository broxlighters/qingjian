...
青简自绘外观正在实验，尚无通过完整桌面验收的应用路径，普通安装下三个选项均保留 Fcitx5 面板。`auto` 暂不启用自绘；`qingjian` 只有在额外启用实验构建时才会尝试 X11/XWayland。原生 Wayland 应用和 GNOME 搜索仍使用 Fcitx5 面板。出现显示问题时改回 `fcitx` 即可恢复默认外观。
