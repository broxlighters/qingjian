...
#include "mapping.h"
namespace qingjian {
nlohmann::json mapKey(const fcitx::Key &key, bool english) {
    ...
    return {{"virtual_key", code}, {"character", character}, {"modifiers", {
        {"ctrl", states.test(fcitx::KeyState::Ctrl)}, {"shift", states.test(fcitx::KeyState::Shift)},
        {"alt", states.test(fcitx::KeyState::Alt)}, {"win", states.test(fcitx::KeyState::Super) || states.test(fcitx::KeyState::Hyper)},
        {"caps", states.test(fcitx::KeyState::CapsLock)}, {"english_mode", english}}}};
}
}
