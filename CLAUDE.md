...
跨平台输入法，Core 平台无关，各平台只做壳。已发版 macOS 0.1.2（自用 + 测试者）、Windows 0.1.0-alpha.2（内测）；Linux 已接入 Fcitx5 本地输入链路，仍待桌面兼容性验证。
...
- `apps/linux`：`server`（Rust Engine + Unix socket 会话）+ `fcitx5`（C++ 默认候选 UI 插件）；安装/验证见 `docs/notes/linux-fcitx5.md`。
